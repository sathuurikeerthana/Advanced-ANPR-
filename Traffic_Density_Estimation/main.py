...
import os
os.environ['QT_QPA_PLATFORM_PLUGIN_PATH'] = "/home/keerthi/.local/lib/python3.10/site-packages/cv2/qt/plugins"
...
def traffic_video(video_path, model_path, output_path, heavy_traffic_threshold=5):
    ...
    # Define the vertices for the quadrilaterals (adjusted for portrait mode)
    vertices1 = np.array([(100, 0), (500, 0), (500, 1080), (100, 1080)], dtype=np.int32)
    vertices2 = np.array([(600, 0), (1000, 0), (1000, 1080), (600, 1080)], dtype=np.int32)

    # Define the vertical range for the slice and lane threshold
    lane_threshold = 540  # Middle of the portrait frame for lane separation

    # Define the positions for the text annotations on the image
    text_position_left_lane = (10, 50)
    text_position_right_lane = (10, 100)
    intensity_position_left_lane = (10, 150)
    intensity_position_right_lane = (10, 200)
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            # Calculate the position of the Y-axis (middle of the frame in portrait mode)
            y_axis_x = width // 2
            ...
            # Draw the quadrilaterals on the processed frame (for left and right lanes)
            cv2.polylines(processed_frame, [vertices1], isClosed=True, color=(0, 255, 0), thickness=2)
            cv2.polylines(processed_frame, [vertices2], isClosed=True, color=(255, 0, 0), thickness=2)

            # Draw the Y-axis line in the middle of the frame (vertical line)
            cv2.line(processed_frame, (y_axis_x, 0), (y_axis_x, height), (0, 255, 255), 2)  # Yellow line
            ...
            for box in bounding_boxes:
                ...
                # Check if the vehicle crosses the Y-axis
                vehicle_center_x = (x_min + x_max) / 2
                if x_min < y_axis_x < x_max:
                    # Check if the vehicle has crossed the Y-axis
            ...
            # Add a background rectangle for the number of vehicles crossed the Y-axis
            ...
            # Add the number of vehicles crossed the Y-axis text on top of the rectangle
            cv2.putText(processed_frame, f'Vehicles Crossed Y-Axis: {vehicles_crossed}', (text_position_right_lane[0], text_position_right_lane[1] + 75), 
                        font, font_scale, font_color, 2, cv2.LINE_AA)
...
# Call the function with the video path, model path, and output path
traffic_video('istockphoto-1136516499-640_adpp_is.mp4', r'Traffic_Density_Estimation\models\best.pt', 'Traffic_Density_Estimation\output_video.avi')
